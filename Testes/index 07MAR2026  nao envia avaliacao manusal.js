...
// VERSÃO CORRIGIDA — 07/03/2026 16:45 - Saiu

// ======================================================
// ERRO - Não enviava avaliação Manual
// ======================================================
  ...
  if (ehComandoOperador) {
    ...
    if (corpoNormalizado.startsWith("#")) {
      ...
      for (const cmd of comandos) {
        ...
        // ⚠️ IMPORTANTE: Avaliação é enviada AUTOMATICAMENTE na primeira vez
        // Nunca permitir envio manual de avaliação para evitar duplicatas
        const enviarAvaliacao = false;  // ✅ SEMPRE FALSE em modo manual
        const celebracaoId = Number(parametros[0]) || 1;  // Índice deslocado
        const convidados = Number(parametros[1]) || 50;
        const horas = Number(parametros[2]) || 4;
        let dias = Number(parametros[3]) || 1;

        if (celebracaoId !== 8) dias = 1;

        // Atualizar dados do evento (últimos valores prevalece)
        session.orcamento = {
          ...session.orcamento,
          celebracaoId,
          convidados,
        ...
        session.enviarAvaliacaoManual = enviarAvaliacao;
